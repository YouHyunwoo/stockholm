	...
	def print_state(self, investment):
		...
		if logger is not None:
			if self.context.market_start_time is not None:
				...
				logger.info('{:>4s} |  BUY | {}'.format(investment.ticker.code, self.context.buying_price))
				logger.info('{:>4s} | SELL | {}'.format(investment.ticker.code, self.context.selling_price))
